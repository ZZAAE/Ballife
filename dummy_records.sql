...
--  기간: 2026-06-04 ~ 2026-06-11 (8일)  ※ 06-10/06-11 은 데모용 미래 데이터
...
--
--  ▶ 메모
--    - 한 번만 실행하세요. (재실행 시 같은 날짜 기록이 중복 삽입됩니다)
--    - 혈압약 처방은 해당 유저에게 없으면 자동 생성하고, 있으면 재사용합니다.
--    - 운동 exercise_type_id 는 MongoDB 시드 ObjectId 입니다. 다른 머신의 시드가
--      다르면 운동 '종류명'만 안 보일 수 있고(행/소모칼로리는 정상), 기록 삽입은 정상입니다.
...
SELECT @USER_ID, 'MEDICINE', '혈압약', '2026-06-04', 'MORNING,DINNER,BEDTIME', '1정', 0
FROM dual WHERE @PRESCRIPTION_ID IS NULL;
SET @PRESCRIPTION_ID := COALESCE(@PRESCRIPTION_ID, LAST_INSERT_ID());


-- ============================================================
-- 1) 생체 수치 (BIO_VALUE_RECORD) : 2026-06-04 .. 2026-06-11 (8일 x 9건 = 72건)
...
-- ============================================================
-- 2) 식사 (MEAL + MEAL_ITEM) : 2026-06-04 .. 2026-06-11
...
-- ============================================================
-- 3) 복약 기록 (USER_MEDICINE_RECORD) : 2026-06-04 .. 2026-06-11
...
-- ============================================================
-- 4) 운동 (user_exercise) : 2026-06-04 .. 2026-06-11 (8일 중 7세션, 휴식일 포함)
...
-- SELECT 'bio'  k, COUNT(*) c FROM BIO_VALUE_RECORD WHERE user_id=@USER_ID AND record_date BETWEEN '2026-06-04' AND '2026-06-11'
-- UNION ALL SELECT 'meal',      COUNT(*) FROM MEAL WHERE user_id=@USER_ID AND meal_date BETWEEN '2026-06-04' AND '2026-06-11'
-- UNION ALL SELECT 'meal_item', COUNT(*) FROM MEAL_ITEM mi JOIN MEAL m ON m.meal_id=mi.meal_id WHERE m.user_id=@USER_ID AND m.meal_date BETWEEN '2026-06-04' AND '2026-06-11'
-- UNION ALL SELECT 'medicine',  COUNT(*) FROM USER_MEDICINE_RECORD WHERE prescription_id=@PRESCRIPTION_ID AND intake_date BETWEEN '2026-06-04' AND '2026-06-11'
-- UNION ALL SELECT 'exercise',  COUNT(*) FROM user_exercise WHERE user_id=@USER_ID AND exercise_date BETWEEN '2026-06-04' AND '2026-06-11';
