...
public class UserMedalController {
    ...
    @Operation(summary = "포인트 기준 메달 자동 지급 체크", description = "누적 포인트 기준으로 달성 조건을 충족한 메달을 자동으로 지급합니다.")
    @PostMapping("/check")
    public ResponseEntity<List<UserMedalDto.Response>> checkMedals(
            @AuthenticationPrincipal CustomUserDetails principal) {
        if (principal == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
        return ResponseEntity.ok(userMedalService.checkAndGrantMedals(principal.getUserId()));
    }

    @Operation(summary = "메달 장착", description = "보유한 메달을 대표 메달로 장착합니다.")
    @PutMapping("/{medalId}/equip")
    public ResponseEntity<Void> equipMedal(
            @AuthenticationPrincipal CustomUserDetails principal,
            @Parameter(description = "메달 ID") @PathVariable Long medalId) {
        if (principal == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        }
        userMedalService.equipMedal(principal.getUserId(), medalId);
