...
import characteristicIcon from "../../../assets/characteristic.svg";
import aiIcon from "../../../assets/ai.svg";

import allIcon from "../../../assets/all.svg";
import bloodPressureIcon from "../../../assets/bloodpressure.svg";
...
export default function HealthIndicatorMenu({ onRegisterClick }) {
  ...
  return (
    <aside className="w-full rounded-2xl bg-white px-5 py-6 font-['Noto_Sans_KR'] shadow-sm xl:w-[320px] xl:self-start xl:px-[24px] xl:py-[40px]">
      {/* 상단 제목 */}
      ...
      {/* 메뉴 라벨 */}
      <p className="mt-5 ml-2 text-[12px] font-normal leading-none text-[#777777] xl:mt-[22px] xl:ml-[23px]">
        메뉴
      </p>

      {/* 메뉴 리스트 */}
      <nav className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:mt-[22px] xl:flex xl:flex-col xl:gap-[9px]">
        {menuItems.map((item) => {
          ...
          return (
            <button
              ...
              className={[
                "relative flex h-[55px] w-full items-center gap-3 rounded-[5px] text-left transition-none xl:gap-[14px]",
                isActive
                  ? "bg-[#E8E8E8] pl-3 xl:pl-[20px]"
                  : "bg-transparent pl-3 xl:pl-[23px]",
              ].join(" ")}
            >
              {isActive && (
      ...
      <button
        ...
        className="mt-5 h-[55px] w-full rounded-[6px] bg-[#050505] text-[15px] font-bold tracking-[-0.2px] text-white xl:mt-[31px]"
      >
        기록하기
      </button>

      {/* 파란 조언 카드 */}
      <section className="mt-6 rounded-[10px] bg-[#0057E5] px-[24px] pt-[24px] pb-[24px] text-white shadow-[0_18px_32px_rgba(0,0,0,0.14)] xl:mt-[86px]">
