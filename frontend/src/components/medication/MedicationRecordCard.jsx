      ...
      <div className="bg-[#1B1F2A] rounded-2xl p-4 sm:p-5 lg:p-6 text-white flex flex-col">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[15px] sm:text-[16px] font-bold">
            {t("medicationRecordCard.title")}
          </span>
        </div>

        <p className="text-[12px] sm:text-[13px] text-gray-400 leading-relaxed mb-4">
          {t("medicationRecordCard.description")}
        </p>

        <div className="flex flex-col sm:flex-row gap-3 mb-3">
          <div className="flex-1">
            <label className="text-[11px] sm:text-[12px] text-gray-400 mb-1.5 block">
              {t("medicationRecordCard.drugNameLabel")}
            </label>
            <input
              type="text"
              placeholder={t("medicationRecordCard.drugNamePlaceholder")}
              value={drugName}
          ...
          <div className="flex-1">
            <label className="text-[11px] sm:text-[12px] text-gray-400 mb-1.5 block">
              {t("medicationRecordCard.dosageLabel")}
            </label>
            <input
              type="text"
              value={dosage}
              onChange={(e) => setDosage(e.target.value)}
              placeholder={t("medicationRecordCard.dosagePlaceholder")}
              className="w-full h-[40px] sm:h-[42px] bg-[#2A2F3F] rounded-lg px-4 text-[12px] sm:text-[13px] text-white placeholder-gray-500 outline-none"
            />
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          <div className="flex-1">
            <label className="text-[11px] sm:text-[12px] text-gray-400 mb-1.5 block">
              {t("medicationRecordCard.dateLabel")}
            </label>
            <input
              type="date"
              value={date}
          ...
          <div className="flex-1">
            <label className="text-[11px] sm:text-[12px] text-gray-400 mb-1.5 block">
              {t("medicationRecordCard.timeLabel")}
            </label>
            <input
              type="time"
              value={time}
