...
SYSTEM_PROMPT = """You are 'Ball', the health AI assistant for Ballife.
A summary of the user's recent health data is provided with every message. Give personalized advice based on this data.
...
- Never switch to a different language unless the user explicitly asks you to.
...
[답변 규칙]
- 사용자가 질문한 언어로 답변 (영어 질문 → 영어 답변, 일본어 질문 → 일본어 답변)
- 2~4문장으로 간결하게
- 이모지 사용 금지
...
async def fetch_user_health_data(
    user_id: int, token: str | None, target_date: date
) -> dict:
    headers = {"Authorization": f"Bearer {token}"} if token else {}
    async with httpx.AsyncClient() as client:
        results = await asyncio.gather(
            client.get(
                f"{SPRING_API}/api/bioValueRecords/search/{user_id}", headers=headers
            ),
            # 식단은 요청한 날 하루치만 조회 (Meal 단위)
            client.get(
                f"{SPRING_API}/api/meal/today",
                params={"userId": user_id, "date": target_date.isoformat()},
                headers=headers,
            ),
        ...
        meals_with_id = [
            m for m in meals if isinstance(m, dict) and m.get("mealId") is not None
        ]
        item_results = await asyncio.gather(
            *[
                client.get(
                    f"{SPRING_API}/api/mealItem/byMeal/{m['mealId']}", headers=headers
                )
...
def _summarize_day_meals(meals, target_date) -> list[str]:
    ...
    meal_lines = []
    for m in meals:
        cat = MEAL_CATEGORY_KR.get(
            str(m.get("mealCategory")), str(m.get("mealCategory") or "기타")
        )
        ...
        day_cal += meal_cal
        meal_lines.append(
            f"{head}: " + ", ".join(food_parts) + f" (끼니 합계 {_fmt(meal_cal)}kcal)"
        )
